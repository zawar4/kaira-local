...
class ProcessAssessmentProfiles @Inject constructor(private val assessmentRepository: AssessmentRepository, viewModelCoroutineScope: CoroutineScope) : BaseUseCase(viewModelCoroutineScope) {
    ...
    operator fun invoke(languageLocale:String,userId: String, financialAssessmentProfile: FinancialProfile, psychologicalAssessmentProfile: PsychologicalProfile): MutableLiveData<Result<Strategy>> {
        return processAssessmentProfiles(languageLocale,userId,financialAssessmentProfile,psychologicalAssessmentProfile)
    }

    private fun processAssessmentProfiles(languageLocale:String,userId: String, financialAssessmentProfile: FinancialProfile, psychologicalAssessmentProfile: PsychologicalProfile): MutableLiveData<Result<Strategy>>{
